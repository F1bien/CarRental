		...
		while (!exit) {
			...
			switch (input) {
				...
				case 3:
					...
					if (num == 1) {
						System.out.print(
								"Insert the cars data licensePlate, brand, model, cilindrata, anno, seats, price:\t");
						rental.addCar(new Car(scanner.next(), scanner.next(), scanner.next(), scanner.nextInt(),
								scanner.next(), scanner.nextInt(), scanner.nextFloat()));
					}
					break;
